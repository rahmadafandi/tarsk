...
pub fn new_shared(broker: Broker, total: usize, batch: bool) -> Arc<Shared> {
    Arc::new(Shared {
        broker,
...
pub fn io_err(err: Box<dyn std::error::Error + Send + Sync>) -> io::Error {
    io::Error::other(err.to_string())
}

/// Scratch directory for the child socket. Torn down with the run.
pub fn socket_dir() -> io::Result<std::path::PathBuf> {
